function [xk, workingSet, lagMult, innerIt, errorRecursive] = qpasCholesky(L,f,C,d,x0,workingSet,maxiter, ...
...
if isempty(workingSet)      %Cold start
    ...
    Ginv_Ckt = linsolve(L', ...
        linsolve(L,(C(workingSet,:)'), struct('LT', true)), ...
        struct('UT', true));            % Helper G^{-1}C_{W_k}^T
    schur = C(workingSet,:)*Ginv_Ckt;   % Schur Complement
    [Q,R] = qr(schur);                  % QR factors
    rhs = C(workingSet,:)*(-pk);        % Right-hand side of the QPAS system
        
    lagMult = -linsolve(R,(Q'*rhs),struct('UT', true));     % Lagrange multipliers
...
while true
    ...
    if norm(pk) > EPSzero
        ...
        if alpha<=1                             % Blocking constraint
            ...
            Ginv_Ckt = [Ginv_Ckt, linsolve(L',linsolve(L,(C(bcidx,:)'), struct('LT', true)), struct('UT', true))];

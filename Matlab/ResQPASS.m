function [x,y,V,res,iters,obj,recursiveError,X] = ResQPASS(A,b,l,u, maxInnerIt, M1, maxOuterIt, doWarmStart, doRecursiveQPAS, resTol)
...
for outerIt = 1:maxOuterIt
    ...
    if nargout <= 6         % Standard QPAS
        [y,workingSet,lagMultActive,iters(outerIt)] = ...
            qpasCholesky(L,f,VV,lu,[y;0],workingSet,maxInnerIt,doRecursiveQPAS);
    else                    % QPAS with computation of recursion error
        [y,workingSet,lagMultActive,iters(outerIt),recursiveErrorIter] = ...
            qpasCholesky(L,f,[-V;V],[-l,u],[y;0],workingSet,maxInnerIt,doRecursiveQPAS);
